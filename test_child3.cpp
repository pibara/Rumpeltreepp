...
int main(int argc,char **argv) {
  ...
  auto domainnode1 = childnode1["David"];
  auto domainnode2 = childnode2["David"];
  if (domainnode1.cap() == domainnode2.cap()) {
      std::cout << "Same child from same name with different ancester trees, this is very bad." << std::endl;
      return 1;
  }
  return 0;
};
